...
class MoeGroupedMatmulBwdOperationDescriptor
    : public HipdnnBackendDescriptorImpl<MoeGroupedMatmulBwdOperationDescriptor>,
...
public:
    ...
    const hipdnn_flatbuffers_sdk::data_objects::MoeGroupedMatmulBwdAttributesT& getData() const
    {
        return _data;
    }

    // Access to tensor descriptor references for graph building
    std::shared_ptr<TensorDescriptor> getDoutputDesc() const
    {
        return _doutputDesc;
    }
    ...
    std::shared_ptr<TensorDescriptor> getDweightDesc() const
    {
        return _dweightDesc;
    }
    ...
    // Creates a finalized MoeGroupedMatmulBwdOperationDescriptor directly from a FlatBuffer NodeT.
    // Casts nodeT.attributes to MoeGroupedMatmulBwdAttributes internally, then directly assigns
    // the data struct, looks up tensor descriptors from the tensor map, and calls finalize().
    static std::shared_ptr<MoeGroupedMatmulBwdOperationDescriptor>
