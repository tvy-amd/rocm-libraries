...
"""Regenerate hipDNN FlatBuffers C++ headers from edited .fbs schemas.

Runs `flatc --cpp` for each changed schema, writing the data-object headers
(data_objects/*_generated.h) into the committed include tree. This is the
pre-commit entry point (hook `flatc-hipdnn`).

Scope limit: this script does NOT regenerate the UMD op-schema registry
(umd/op_schema_registry_generated.hpp). That header is derived from a compiled
C++ tool (umd_registry_gen) via the FlatBuffers reflection schema (graph.bfbs),
which the pre-commit environment cannot build. It is regenerated by the CMake
build (HIPDNN_GENERATE_SDK_HEADERS, default ON) and enforced in CI by the
"Verify UMD op-schema registry is in sync" step in hipdnn-superbuild-ci.yml.
"""
