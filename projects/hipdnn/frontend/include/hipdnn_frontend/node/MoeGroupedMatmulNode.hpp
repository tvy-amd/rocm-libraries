...
public:
    ...
    Error infer_properties_node() override
    {
        ...
        const auto tokenCount = attributes.get_mode() == MoeGroupedMatmulMode::GATHER
                                    ? attributes.get_token_index()->get_dim()[1]
                                    : token->get_dim()[1];
        const std::vector<int64_t> expectedOutputDims = {1, tokenCount, weight->get_dim()[2]};
        ...
        else
        {
            HIPDNN_RETURN_IF_NE(
                ...
                expectedOutputDims,
                ErrorCode::INVALID_VALUE,
                "MoE output tensor dimensions do not match the inferred dimensions");
        }
