...
THRUST_HOST MR* get_per_device_resource(const thrust::detail::execution_policy_base<DerivedPolicy>& system)
{
...
public:
   ...
   */
  THRUST_HOST per_device_allocator()
      : base(get_per_device_resource<Upstream>(ExecutionPolicy()))
  {}

  /*! Copy constructor. Copies the memory resource pointer. */
  THRUST_HOST_DEVICE per_device_allocator(const per_device_allocator& other)
      : base(other)
  {}

  /*! Conversion constructor from an allocator of a different type. Copies the memory resource pointer. */
  template <typename U>
  THRUST_HOST_DEVICE per_device_allocator(const per_device_allocator<U, Upstream, ExecutionPolicy>& other)
      : base(other)
  {}

  /*! Destructor. */
  THRUST_HOST_DEVICE ~per_device_allocator() {}
};
