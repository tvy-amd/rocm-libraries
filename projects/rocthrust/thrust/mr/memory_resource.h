...
public:
   ...
   */
  THRUST_NODISCARD pointer allocate(std::size_t bytes, std::size_t alignment = THRUST_MR_DEFAULT_ALIGNMENT)
  {
   ...
   */
  THRUST_HOST_DEVICE bool is_equal(const memory_resource& other) const noexcept
  {
   ...
   *  \return whether the two resources are equivalent.
   */
  THRUST_HOST_DEVICE virtual bool do_is_equal(const memory_resource& other) const noexcept
  {
    return this == &other;
  }
};
...
THRUST_HOST_DEVICE bool operator==(const memory_resource<Pointer>& lhs, const memory_resource<Pointer>& rhs) noexcept
{
...
THRUST_HOST MR* get_global_resource()
{
  static MR resource;
  return &resource;
}
