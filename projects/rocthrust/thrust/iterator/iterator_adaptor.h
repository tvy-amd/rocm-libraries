...
public:
  ...
  THRUST_HOST_DEVICE explicit iterator_adaptor(Base const& iter)
      : m_iterator(iter)
  {}
   ...
   */
  THRUST_HOST_DEVICE Base const& base() const
  {
    return m_iterator;
  }
...
protected:
  /*! \return A \p const reference to the \p Base iterator this \p iterator_adaptor adapts.
   */
  THRUST_HOST_DEVICE Base const& base_reference() const
  {
    return m_iterator;
  }
   ...
   */
  THRUST_HOST_DEVICE Base& base_reference()
  {
    return m_iterator;
  }
...
private: // Core iterator interface for iterator_facade
  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST_DEVICE typename iterator_adaptor::reference dereference() const
  {
    return *m_iterator;
  }
  ...
  THRUST_HOST_DEVICE bool equal(iterator_adaptor<OtherDerived, OtherIterator, V, S, T, R, D> const& x) const
  {
    return m_iterator == x.base();
  }

  THRUST_EXEC_CHECK_DISABLE
  THRUST_HOST_DEVICE void advance(typename iterator_adaptor::difference_type n)
  {
  ...
  THRUST_EXEC_CHECK_DISABLE
  template <typename OtherDerived, typename OtherIterator, typename V, typename S, typename T, typename R, typename D>
  THRUST_HOST_DEVICE typename iterator_adaptor::difference_type
