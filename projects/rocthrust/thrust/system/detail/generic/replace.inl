...
struct new_value_if
{
  THRUST_HOST_DEVICE new_value_if(Predicate p, NewType nv)
      : pred(p)
      , new_value(nv)
  {}

  template <typename InputType>
  THRUST_HOST_DEVICE OutputType operator()(const InputType& x) const
  {
  ...
  THRUST_HOST_DEVICE OutputType operator()(const InputType& x, const PredicateArgumentType& y)
  {
...
{
  THRUST_HOST_DEVICE constant_unary(T _c)
      : c(_c)
  {}

  template <typename U>
  THRUST_HOST_DEVICE T operator()(U&)
  {
    return c;
  } // end operator()()
...
THRUST_HOST_DEVICE OutputIterator replace_copy_if(
...
THRUST_HOST_DEVICE OutputIterator replace_copy(
  thrust::execution_policy<DerivedPolicy>& exec,
  InputIterator first,
  InputIterator last,
  OutputIterator result,
...
THRUST_HOST_DEVICE void replace_if(
  thrust::execution_policy<DerivedPolicy>& exec,
