...
public:
  ...
  THRUST_HOST_DEVICE index_range(index_type begin, index_type end)
      : m_begin(begin)
      , m_end(end)
  {}
...
public:
  ...
  THRUST_HOST_DEVICE uniform_decomposition(index_type N, index_type granularity, index_type max_intervals)
      : m_N(N)
  ...
  THRUST_HOST_DEVICE index_range<index_type> operator[](const index_type& i) const
  {
    if (i < m_threshold)
    {
