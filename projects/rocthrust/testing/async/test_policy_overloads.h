...
struct test_policy_overloads
{
  ...
  static constexpr std::size_t num_postfix_arg_sets = std::tuple_size<postfix_args_type>::value;
...
private:
  ...
  static void test_postfix_overloads(std::size_t const num_values, size_const<PostfixIdx> = {})
  {
  ...
  static void test_postfix_overloads(std::size_t const, size_const<num_postfix_arg_sets>)
  {
    // terminal case, no-op
  }
...
#  endif
    ...
    basic_policy_test<PostfixIdx>("(no policy)", std::make_tuple(), using_default_stream, num_values);

    basic_policy_test<PostfixIdx>("thrust::device", std::make_tuple(thrust::device), using_default_stream, num_values);
    ...
    basic_policy_test<PostfixIdx>(
      ...
      std::make_tuple(thrust::device.on(test_stream.get())),
      using_test_stream,
      num_values);

    basic_policy_test<PostfixIdx>(
      "thrust::device(thrust::device_allocator<void>{}).on(test_stream.get())",
      std::make_tuple(thrust::device(thrust::device_allocator<void>{}).on(test_stream.get())),
      using_test_stream,
      num_values);
  }
  ...
  // the results.
  template <std::size_t PostfixIdx, typename PrefixArgTuple, typename ValidateEvent>
  static void basic_policy_test(
    std::string const& policy_desc,
  ...
  try
  {
    ...
    // and events/futures:
    auto e_a =
      algo_def::invoke_async(prefix_tuple, prefix_index_seq{}, input_a, output_a, postfix_tuple, postfix_index_seq{});
    auto e_b =
      algo_def::invoke_async(prefix_tuple, prefix_index_seq{}, input_b, output_b, postfix_tuple, postfix_index_seq{});
    auto e_c =
      algo_def::invoke_async(prefix_tuple, prefix_index_seq{}, input_c, output_c, postfix_tuple, postfix_index_seq{});
    auto e_d =
      algo_def::invoke_async(prefix_tuple, prefix_index_seq{}, input_d, output_d, postfix_tuple, postfix_index_seq{});

    // Let reference calc overlap with async testing:
    algo_def::invoke_reference(input_ref, output_ref, postfix_tuple, postfix_index_seq{});
  ...
  try
  {
    ...
    auto e_a = algo_def::invoke_async(
      std::make_tuple(thrust::device), prefix_index_seq{}, input_a, output_a, postfix_tuple, postfix_index_seq{});
...
#  endif
    ...
    auto e_b = algo_def::invoke_async(
    ...
    auto e_c                    = algo_def::invoke_async(
      policy_after_e_b_tuple, prefix_index_seq{}, input_c, output_c, postfix_tuple, postfix_index_seq{});
    ...
    ASSERT_THROWS_EQUAL(
      auto x = algo_def::invoke_async(
        policy_after_e_b_tuple, prefix_index_seq{}, input_tmp, output_tmp, postfix_tuple, postfix_index_seq{});
      THRUST_UNUSED_VAR(x), thrust::event_error, thrust::event_error(thrust::event_errc::no_state));
  ...
  catch (unittest::UnitTestException& exc)
  {
    ...
    std::string const postfix_desc = unittest::demangle(typeid(postfix_t).name());
    std::string const input_desc   = unittest::demangle(typeid(input_type).name());
    std::string const output_desc  = unittest::demangle(typeid(output_type).name());
