...
################################################################################
"""Fixtures for the SolutionArms characterization suite.

Provides fully-derived Solution states from the vendored gfx942 HSS and BBS
logic fixtures, plus the cap map and assembler, so the arm-sweep tests can
mutate individual parameters and directly call Solution's static methods.
"""
...
@pytest.fixture
def bbs_state(_base_solutions):
    """Deep copy of the gfx942 BBS solution state (safe to mutate)."""
    return copy.deepcopy(_base_solutions.get("gfx942_BBS", _base_solutions["gfx942_HSS"]))
