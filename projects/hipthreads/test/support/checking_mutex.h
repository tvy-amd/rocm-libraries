...
struct checking_mutex {
  ...
  __host__ __device__ checking_mutex()                      = default;
  __host__ __device__ checking_mutex(const checking_mutex&) = delete;
  __host__ __device__ ~checking_mutex() { assert(current_state == unlocked); }
  ...
  __host__ __device__ bool try_lock_for(const std::chrono::duration<Rep, Period>&) {
  ...
  __host__ __device__ bool try_lock_until(const std::chrono::time_point<Clock, Duration>&) {
